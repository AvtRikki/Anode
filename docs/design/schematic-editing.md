...
- **Stage 5** — started: nets built from wires, junctions, labels of every kind (a label or a pin landing part way
  ...
  design from KiCad's own tables, and the netlist is exported in KiCad's format. A sheet symbol's pins and the hierarchical labels inside it are
  checked against each other. BOM export is not done. A net can be lit (backquote, the
  ...
  nets, sheets joined through their pins and their global names, each with the pins it reaches anywhere.
- **Stage 6** — not started.
