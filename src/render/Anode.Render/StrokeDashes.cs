...
public static class StrokeDashes
{
    ...
    /// line too short to show a dash still has to be visible, and KiCad draws one too.
    /// </summary>
    public static IEnumerable<(Vector2 From, Vector2 To)> Cut(Vector2 a, Vector2 b, float[] pattern)
    {
        ...
        {
            float run = pattern[step % pattern.Length];
            if (run <= 0)
            {
                continue;
            }
            ...
            if (step % 2 == 0)
            {
                yield return (a + (along * at), a + (along * Math.Min(at + run, length)));
            }
