...
public static class SchematicSceneBuilder
{
    ...
    private sealed class Builder(SchematicScene scene)
    {
        ...
        public void AddSheetFrame(Vector2L paper)
        {
            ...
            const long margin = 10_000_000;
            Vector2D[] frame =
            [
                new(margin, margin), new(paper.X - margin, margin),
                new(paper.X - margin, paper.Y - margin), new(margin, paper.Y - margin),
            ];

            for (int i = 0; i < frame.Length; i++)
            {
                Line(LayerStyle.Sch.Symbol, frame[i], frame[(i + 1) % frame.Length], SymbolWidth, OutlineLoops.NoOwner);
            }
        }
