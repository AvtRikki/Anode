...
/// <summary>
/// Draws an <see cref="IRenderScene"/> with OpenGL 3.3 / ES 3.0. Each layer is uploaded once into instance buffers
...
public sealed unsafe class GlSceneRenderer : IDisposable
{
    ...
    public void SetScene(IRenderScene? scene)
    {
    ...
    /// <summary>
    /// Dots at the grid crossings (kit: a dot grid). A dot is a zero-length segment: the segment shader draws round
    /// caps, so one instance per crossing is enough.
    /// </summary>
    private void DrawGrid(ViewState view, RectD visible)
    {
        double spacing = view.GridSpacing;
        float dot = (float)(1.6 / view.PixelsPerMm);
        ...
        for (double x = Math.Floor(visible.MinX / spacing) * spacing; x <= visible.MaxX; x += spacing)
        {
            ...
            {
                _dynamic.AddRange([(float)x, (float)y, (float)x, (float)y, dot]);
            }
        }

        UploadDynamicSegments(_grid);
        Draw(_grid, LayerStyle.Grid, view, Transform2D.Identity);
    }
